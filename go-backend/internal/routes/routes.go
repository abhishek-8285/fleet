...
func RegisterRoutes(router *gin.RouterGroup, services *services.Container) {
	// Initialize handlers
	authHandler := handlers.NewAuthHandler(services)
	driverHandler := handlers.NewDriverHandler(services)
	vehicleHandler := handlers.NewVehicleHandler(services)
	tripHandler := handlers.NewTripHandler(services)
	fuelHandler := handlers.NewFuelHandler(services)
	uploadHandler := handlers.NewUploadHandler(services)
	locationHandler := handlers.NewLocationHandler(services)
	analyticsHandler := handlers.NewAnalyticsHandler(services)
	whatsappHandler := handlers.NewWhatsAppHandler(services)

	// JWT Middleware
	jwtMiddleware := middleware.JWTMiddleware(services.JWTService)
	...
	protected.Use(jwtMiddleware)
	{
		...
		{
			mqttHandler := handlers.NewMQTTHandler(services)
